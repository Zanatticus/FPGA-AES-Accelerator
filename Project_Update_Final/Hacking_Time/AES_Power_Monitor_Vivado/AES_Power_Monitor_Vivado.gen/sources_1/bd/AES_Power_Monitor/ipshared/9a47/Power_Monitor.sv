...
module Power_Monitor(clk, enable, avg_ro_freq);
    ...
    output reg [31:0] avg_ro_freq;
    
    wire [999:0] enables, clks;
    reg [31:0] power_readings [999:0];
    
    one_to_100 enable_exapand(.in(enable), .out(enables));
    one_to_100 clk_expand(.in(clk), .out(clks));
    freq_ro RO[999:0](.clk(clks), .ro_enable(enables), .freq(power_readings));
...
module calc_avg(power_readings, power_avg);
    input  [31:0] power_readings [999:0];
